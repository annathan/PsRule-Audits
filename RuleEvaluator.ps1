# RuleEvaluator.ps1
# Explicit rule evaluation engine with verifiable logic
...
Write-Host "🔍 Starting explicit rule evaluation..." -ForegroundColor Cyan
...
function Test-Evaluator {
    ...
    if ($ExpectedType -and $Value -ne $null) {
        ...
        if ($ExpectedType -eq "array" -and $Value -is [array]) {
            $ActualType = "array"
        } elseif ($ExpectedType -eq "object" -and $Value -is [PSCustomObject]) {
            $ActualType = "object"
        } elseif ($ExpectedType -eq "string" -and $Value -is [string]) {
            $ActualType = "string"
        } elseif ($ExpectedType -eq "boolean" -and $Value -is [bool]) {
            $ActualType = "boolean"
        } elseif ($ExpectedType -eq "number" -and ($Value -is [int] -or $Value -is [double] -or $Value -is [decimal])) {
            $ActualType = "number"
        }
    ...
    switch ($Evaluator) {
        ...
        "type" {
            $ActualType = if ($Value -is [array]) { "array" } elseif ($Value -is [PSCustomObject]) { "object" } else { $Value.GetType().Name.ToLower() }
            return @{
