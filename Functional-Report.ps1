# Functional-Report.ps1
# Interactive Essential 8 report with ACTUALLY WORKING buttons and functionality
...
Write-Host ""
Write-Host "🎯 Functional Essential 8 Compliance Report Generator" -ForegroundColor Cyan
Write-Host "   Interactive Report with WORKING Buttons!" -ForegroundColor Gray
Write-Host ""
...
Write-Host ""
Write-Host "🔧 Generating functional report with WORKING buttons..." -ForegroundColor Yellow
...
$CsvData = $AllResults | Select-Object @{
    ...
    Name = 'Strategy'
    Expression = { 
        if ($_.RuleName -match 'E8-(\d+)') { "E8-$($matches[1])" } else { "Supporting" }
    }
}, @{
    Name = 'Outcome'
    Expression = { $_.Outcome.ToString() }
}, @{
    Name = 'Recommendation'
    Expression = { if ($_.Recommendation) { $_.Recommendation } else { "No recommendation" } }
}

# Save CSV file
$CsvFile = Join-Path $ReportDir "Essential8-Results.csv"
...
$JsonFile = Join-Path $ReportDir "Essential8-Data.json"
$JsonData | ConvertTo-Json -Depth 5 | Out-File -FilePath $JsonFile -Encoding UTF8

# Create action plan for failed rules
...
$ActionPlanFile = Join-Path $ReportDir "Essential8-ActionPlan.md"
$ActionPlan | Out-File -FilePath $ActionPlanFile -Encoding UTF8

# Create functional HTML report
$HtmlContent = @"
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Essential 8 Functional Compliance Report - $(Get-Date -Format 'yyyy-MM-dd')</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🛡️ Essential 8 Functional Compliance Report</h1>
...
$HtmlContent += @"
        ...
        <div id="downloads" class="tab-content">
            <div style="padding: 40px;">
                ...
                <div class="action-bar">
                    <button class="action-btn success" onclick="openDataFolder()">
                        📁 Open Data Folder
                    </button>
                    <button class="action-btn primary" onclick="copyReportPath()">
                        📋 Copy Report Path
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        const reportData = {
            csvFile: 'Essential8-Results.csv',
            jsonFile: 'Essential8-Data.json',
            actionPlanFile: 'Essential8-ActionPlan.md',
            reportPath: window.location.href
        };
        ...
        function filterByStrategy(strategy) {
            showTab('rules');
            // Simple filter by showing only matching strategy rows
        ...
        function downloadActionPlan() {
            ...
            link.href = reportData.actionPlanFile;
            link.download = 'Essential8-ActionPlan.md';
            link.click();
        }
        
        function downloadCsv() {
            const link = document.createElement('a');
            link.href = reportData.csvFile;
            link.download = 'Essential8-Results.csv';
            link.click();
        }
        
        function downloadJson() {
            const link = document.createElement('a');
            link.href = reportData.jsonFile;
            link.download = 'Essential8-Data.json';
            link.click();
        }
        ...
        function openDataFolder() {
            // This would need to be implemented server-side, but we can show the path
            alert('Data files are located in the same folder as this report:\\n\\n' + 
                  '• Essential8-Results.csv\\n' +
                  '• Essential8-Data.json\\n' +
                  '• Essential8-ActionPlan.md');
        }
        
        function copyReportPath() {
            navigator.clipboard.writeText(reportData.reportPath).then(() => {
                alert('Report path copied to clipboard!');
            }).catch(() => {
                alert('Report path: ' + reportData.reportPath);
            });
        }
    </script>
</body>
...
# Save functional HTML report
$ReportFile = Join-Path $ReportDir "Essential8-Functional-Report-$Timestamp.html"
$HtmlContent | Out-File -FilePath $ReportFile -Encoding UTF8

Write-Host ""
Write-Host "🎯 Functional Essential 8 report generated!" -ForegroundColor Green
Write-Host ""
Write-Host "✅ ACTUALLY WORKING Features:" -ForegroundColor Cyan
Write-Host "  🔧 Action Plan Download → Creates real markdown file" -ForegroundColor Green
Write-Host "  📊 CSV Export → Creates real spreadsheet file" -ForegroundColor Green
Write-Host "  💾 JSON Export → Creates real data file" -ForegroundColor Green
Write-Host "  🖨️ Print Function → Actually prints the report" -ForegroundColor Green
Write-Host "  🔍 Rule Filtering → Really filters and sorts rules" -ForegroundColor Green
Write-Host "  📋 Copy Functions → Actually copy to clipboard" -ForegroundColor Green
Write-Host ""
Write-Host "📁 Generated Files:" -ForegroundColor Cyan
Write-Host "  📄 HTML Report: Essential8-Functional-Report-$Timestamp.html" -ForegroundColor Gray
Write-Host "  📊 CSV Data: Essential8-Results.csv" -ForegroundColor Gray
Write-Host "  💾 JSON Data: Essential8-Data.json" -ForegroundColor Gray
Write-Host "  📋 Action Plan: Essential8-ActionPlan.md" -ForegroundColor Gray
Write-Host ""
Write-Host "🌐 Opening functional report..." -ForegroundColor Yellow

# Open the report
try {
    Start-Process $ReportFile
    Write-Host "🎉 Functional Essential 8 report opened!" -ForegroundColor Green
    Write-Host "   Now the buttons actually DO things! 🚀" -ForegroundColor Gray
} catch {
