# AdvancedRuleEvaluator.ps1
# Extended rule evaluation engine with composite conditions, dependencies, and custom evaluators
...
Write-Host "🔍 Starting advanced rule evaluation..." -ForegroundColor Cyan
...
# Enhanced JSON path resolution with nested array support
function Get-JsonPathValue {
    ...
    try {
        ...
        # Handle complex nested array access like Users[0].Roles[1].Name
        if ($Path -match '^(.+)\[(\d+)\]\.(.+)$') {
        ...
        # Handle simple array indexing like [0] or [*]
        if ($Path -match '^(.+)\[(\d+)\]$') {
            $ParentPath = $matches[1]
            $Index = [int]$matches[2]
            $ParentValue = Get-JsonPathValue -Data $Data -Path $ParentPath
...
# Enhanced evaluator with custom evaluator support
...
function Process-Rules {
    ...
    $SortedRules = @()
    $RemainingRules = $Rules | ForEach-Object { $_ }
    
    while ($RemainingRules.Count -gt 0) {
        $ReadyRules = @()
        
        foreach ($Rule in $RemainingRules) {
            if (-not $Rule.dependsOn -or (Test-RuleDependencies -Dependencies $Rule.dependsOn -RuleResults $RuleResults)) {
                $ReadyRules += $Rule
            }
        }
        
        if ($ReadyRules.Count -eq 0) {
            # Handle circular dependencies or missing dependencies
            $ReadyRules = $RemainingRules[0..0]
        }
        
        foreach ($Rule in $ReadyRules) {
            $ProcessedRules += $Rule
            $RemainingRules = $RemainingRules | Where-Object { $_.id -ne $Rule.id }
...
if ($Parallel -and $InputData.Count -gt 1) {
    Write-Host "  🚀 Using parallel processing..." -ForegroundColor Yellow
    $Jobs = @()
    
    foreach ($Record in $InputData) {
        $Job = Start-Job -ScriptBlock {
            param($Data, $RulesPath)
            $Rules = Get-Content $RulesPath | ConvertFrom-Json
            # Import the functions (simplified for job context)
            # This is a simplified version - in production, you'd need to pass the functions
            return "Processed record: $($Data.UserPrincipalName)"
        } -ArgumentList $Record, $RulesJsonPath
        $Jobs += $Job
    }
    ...
    Write-Host "  ⚠️  Parallel processing not fully implemented, using sequential..." -ForegroundColor Yellow
}
...
Write-Host ""
Write-Host "📊 Advanced Evaluation Complete!" -ForegroundColor Green
